...
def group():
    ...
    if current_user.has_role('Admin'):
        ...
        person = db.session.query(ormPersons).filter(ormPersons.id == current_user.id).one_or_none()
        for i in result:
            if len(person.group) > 0:
                if person.group[0] == i:
...
def post(hashtag_name=None):
    if hashtag_name == None:
        ...
        incriment = db.session.query(ormHashtags).filter(ormHashtags.hashtag_name == hashtag_name).one_or_none()
...
def correlation(hashtag_name):
    ...
    if len(result_1) == 0:
        return render_template('regretion.html', error="This tag has no posts")
...
@app.route('/clustering/<hashtag_name>', methods=['GET', 'POST'])
@app.route('/clustering', methods=['GET', 'POST'])
@login_required
@roles_accepted("User", "Admin")
def claster(hashtag_name):
    res1 = db.session.query(ormHashtags).all()
    pop = db.session.query(ormHashtags.hashtag_views).filter(ormHashtags.hashtag_name == hashtag_name).one()[0]
    ...
    for i in matrix_data:
        if kmeans.predict(i)[0] == 0:
            count_elements[0] += 1
            unpopular.append([res1[iter].hashtag_name, res1[iter].hashtag_views])
        else:
            count_elements[1] += 1
            popular.append([res1[iter].hashtag_name, res1[iter].hashtag_views])
        iter += 1
    ...
    return render_template('clasteresation.html', count_claster=count_clasters, graphsJSON=graphsJSON,
                           populatin="Teg " + hashtag_name + " is " + res)
